...
TRAIN_DIR = '/home/xu/MyData/archive/train_images'
TEST_DIR = '/home/xu/MyData/archive/test_images'
...
df = pd.read_csv('/home/xu/MyData/archive/train.csv')
...
torch.save(trunk.state_dict(),'./models/trunk_best.pth')

# best_trunk_weights = glob.glob('./models/{}/trunk_best*.pth'.format(MODEL_NAME))[0]
trunk.load_state_dict(torch.load('./models/trunk_best.pth'))

torch.save(embedder.state_dict(),'./models/embedder.pth')

embedder.load_state_dict(torch.load('./models/embedder.pth'))
...
test_df = pd.read_csv('/home/xu/MyData/archive/sample_submission.csv')
